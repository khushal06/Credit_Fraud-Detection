...
def save_metrics_comparison(lr_metrics, xgb_metrics, output_path: Path):
    metric_keys = ["precision", "recall", "f1", "roc_auc", "pr_auc"]
    metric_labels = ["Precision", "Recall", "F1", "ROC-AUC", "PR-AUC"]
    lr_values = [lr_metrics[k] for k in metric_keys]
    xgb_values = [xgb_metrics[k] for k in metric_keys]

    x = np.arange(len(metric_keys))
    width = 0.35

    fig, ax = plt.subplots(figsize=(9, 5))
    bars_lr = ax.bar(x - width / 2, lr_values, width, label="Logistic Regression", color=COLOR_LR)
    bars_xgb = ax.bar(x + width / 2, xgb_values, width, label="XGBoost", color=COLOR_XGB)
...
def train_and_evaluate():
    ...
    save_confusion_matrices(
        [("Logistic Regression", y_test, lr_pred), ("XGBoost", y_test, xgb_pred)],
        FIGURES_DIR / "confusion_matrices.png",
    )
    save_roc_pr_curves(
        y_test,
        [("Logistic Regression", lr_prob, COLOR_LR), ("XGBoost", xgb_prob, COLOR_XGB)],
        fraud_rate,
        FIGURES_DIR / "roc_pr_curves.png",
    )
    save_metrics_comparison(lr_metrics, xgb_metrics, FIGURES_DIR / "metrics_comparison.png")
